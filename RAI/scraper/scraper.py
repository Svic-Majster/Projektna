...
API_URL = "http://localhost:3000/api/external/data"
...
def scrape_arso_basics():
    ...
    try:
        ...
        for met_data in postaje:
            ...
            try:
                api_response = requests.post(API_URL, json=payload, timeout=5)
                if api_response.status_code == 201:
                    uspesno_shranjenih += 1
                    print(f"{uspesno_shranjenih} > Uspesno posodobljeno: {kraj_ime}")
                else:
                    print(f"Streznik zavrnil {kraj_ime} (Status: {api_response.status_code})")
            except requests.exceptions.RequestException as e:
                print(f"Napaka pri posiljanju za {kraj_ime}: {e}")
                break
                
            time.sleep(0.05)
            
        print(f"\nUspesno obdelanih {uspesno_shranjenih} lokacij.")
